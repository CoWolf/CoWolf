...
public class RecognitionRulesGenerator extends AbstractMojo {
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		List<String> rulebaseProjectNames = new ArrayList<String>();
		...
		for (File workspaceProject : workspaceRoot.listFiles()) {
			...
			if (workspaceProject.isDirectory()) {
				...
				File ruleBaseFile = new File(workspaceProject,AbstractProjectRuleBase.RULEBASE_FILE);
		...
		for (File ruleBaseProject : ruleBaseProjects) {
			...
			File editRulesDir = new File(ruleBaseProject, AbstractProjectRuleBase.SOURCE_FOLDER);
			...
			if (editRulesDir.isDirectory()) {
				...
				getLog().warn(
						"Rulebase project \""
								+ ruleBaseProject.getName() + "\" has no directory \"" + AbstractProjectRuleBase.SOURCE_FOLDER + "\".");
