...
public class AnalyzeWizardPage2 extends WizardPage {
	...
	private TreeNode[] getTree() {
		...
		if (resource == null || resource.getContents() == null || resource.getContents().get(0) == null || ((CTMC) resource.getContents().get(0)).getStates() == null) {
			return new TreeNode[]{labelNode, stateNode};
		}
		
		ArrayList<TreeNode> states = new ArrayList<TreeNode>(((CTMC) resource.getContents().get(0)).getStates().size());
		...
		for (State s : ((CTMC) resource.getContents().get(0)).getStates()) {
			...
			for (de.uni_stuttgart.iste.cowolf.model.ctmc.Label l : s.getLabels()) {
